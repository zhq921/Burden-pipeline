...
if(opt$variant_type=="lof_mis"){ # lof_mis，lof+misburden
  output.file <- paste0(dirname(opt$path_name), "/../subgroup/ACAT/", subgroup_name, "/", basename(opt$path_name),
                        ".",subgroup_name,".", opt$transcript_type,".lof_mis.e",opt$external_MAF,".i",opt$internal_MAF,".ACAT.perm")
  ...
  if(opt$variant_type=="lof"){
    output.file <- paste0(dirname(opt$path_name), "/../subgroup/ACAT/", subgroup_name, "/", basename(opt$path_name),
                          ".",subgroup_name, ".", opt$transcript_type,".lof.e",opt$external_MAF,".i",opt$internal_MAF,".ACAT.perm")
    masks <- c("mask1","mask2")
    mask_weight <- c(1, 0.8)
  }else{
    if(opt$variant_type=="syn"){
      output.file <- paste0(dirname(opt$path_name), "/../subgroup/ACAT/", subgroup_name, "/", basename(opt$path_name),
                            ".",subgroup_name, ".", opt$transcript_type,".syn.e",opt$external_MAF,".i",opt$internal_MAF,".ACAT.perm")
      masks <- c("syn")
      mask_weight <- c(1)
    }else{
      if(opt$variant_type=="lof_hc"){
        output.file <- paste0(dirname(opt$path_name), "/../subgroup/ACAT/", subgroup_name, "/", basename(opt$path_name),
                              ".",subgroup_name, ".", opt$transcript_type,".lof_hc.e",opt$external_MAF,".i",opt$internal_MAF,".ACAT.perm")
        masks <- c("mask1")
        mask_weight <- c(1)
      }
    }
...
## ，population struction，1000GPCA3
# if(!is.null(opt$covariate_PCA)){
#   covMat <- read.table(opt$covariate_PCA, header = F, stringsAsFactors = F, sep = " ")
#   rownames(covMat) <- covMat[,2]
#   covMat <- covMat[sam.info$sam_name,-(1:2)]
#   X <- as.matrix(covMat)[,1:3]
# }
